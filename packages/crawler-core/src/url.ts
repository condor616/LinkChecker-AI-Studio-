...
export function isSameOrSubdomain(hostname: string, rootHostname: string): boolean {
  return hostname === rootHostname || hostname.endsWith(`.${rootHostname}`);
}

export function extractFragmentId(urlStr: string): string | null {
  try {
...
export function canonicalizeHref(href: string, baseUrl: string): string | null {
  try {
